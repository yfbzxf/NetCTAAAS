	...
	<div id="main">
		<form action="findAccount.do" method="post">
			...
			<div class="search_add">
				<div>
					身份证：<input type="text" class="text_search" name="idcard_no"
						value="${accountPage.idcard_no }" />
				</div>
				<div>
					姓名：<input type="text" class="width70 text_search" name="real_name"
						value="${accountPage.real_name }" />
				</div>
				<div>
					登录名：<input type="text" class="text_search" name="login_name"
						value="${accountPage.login_name }" />
				</div>
				<div>
				...
				<input type="button" value="增加" class="btn_add"
					onclick="location.href='toAddAccount.do';" />
			</div>
			<!--删除等的操作提示-->
				...
				<table id="datalist">
					...
					<c:forEach items="${accounts }" var="acc">
						<tr>
							...
							<td><a href="<%=path%>/account/detailAccount.do?account_id=${acc.account_id}">${acc.real_name }</a></td>
							<td>${acc.idcard_no }</td>
							...
							<td><fmt:formatDate value="${acc.create_date }"
									pattern="yyyy-MM-dd" /></td>
							<td><fmt:formatDate value="${acc.last_login_time }"
									pattern="yyyy-MM-dd" /></td>
							<td class="td_modi"><c:choose>
									<c:when test="${acc.status==0 }">
										<input type="button" value="暂停" class="btn_pause"
											onclick="pause_account(${acc.account_id });" />
										<input type="button" value="修改" class="btn_modify"
											onclick="location.href='toUpdateAccount.do?account_id=${acc.account_id }';" />
										<input type="button" value="删除" class="btn_delete"
											onclick="delete_account(${acc.account_id });" />
									</c:when>
									<c:when test="${acc.status==1 }">
										<input type="button" value="开通" class="btn_start"
											onclick="start_account(${acc.account_id });" />
										<input type="button" value="修改" class="btn_modify"
											onclick="location.href='toUpdateAccount.do?account_id=${acc.account_id }';" />
										<input type="button" value="删除" class="btn_delete"
											onclick="delete_account(${acc.account_id });" />
				...
				<c:forEach begin="1" end="${accountPage.totalPage }" var="p">
					<c:choose>
						<c:when test="${p==accountPage.currentPage }">
							<a href="findAccount.do?currentPage=${p }" class="current_page">${p
								}</a>
						</c:when>
						<c:otherwise>
