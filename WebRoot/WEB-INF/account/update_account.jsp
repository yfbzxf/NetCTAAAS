...
<body>
	...
	<div id="main">
		...
		<form action="updateAccount.do" method="post" class="main_form">
			...
			<div class="input_info">
				<input type="text" name="idcard_no" value="${account.idcard_no }"
					readonly class="readonly" />
			</div>
			<div class="text_info clearfix">
				<span>登录账号：</span>
			...
			<div class="input_info">
				<input type="text" name="login_name" value="${account.login_name }"
					readonly class="readonly" />
				<!--                
			...
			<div class="input_info">
				<input type="text" name="telephone" class="width200"
					value="${account.telephone }" /> <span class="required">*</span>
			...
			<div class="input_info">
				<input type="text" name="birthdate" value="${account.birthdate }"
					readonly class="readonly" />
			</div>
			<div class="text_info clearfix">
				<span>Email：</span>
			...
			<div class="input_info">
				<select name="occupation">
					...
					<option value="1"
						<c:if test="${account.occupation==1 }">selected</c:if>>干部</option>
					<option value="2"
						<c:if test="${account.occupation==2 }">selected</c:if>>学生</option>
					<option value="3"
						<c:if test="${account.occupation==3 }">selected</c:if>>技术人员</option>
					<option value="4"
						<c:if test="${account.occupation==4 }">selected</c:if>>其他</option>
				</select>
			</div>
			<div class="text_info clearfix">
				<span>性别：</span>
			...
			<div class="input_info fee_type">
				<input type="radio" name="gender" value="1"
					<c:if test="${account.gender==1 }">checked</c:if> id="female"
					onclick="feeTypeChange(1);" /> <label for="female">女</label> <input
					type="radio" name="gender" value="0"
					<c:if test="${account.gender==0 }">checked</c:if> id="male"
					onclick="feeTypeChange(2);" /> <label for="male">男</label>
			</div>
			<div class="text_info clearfix">
				<span>通信地址：</span>
			</div>
