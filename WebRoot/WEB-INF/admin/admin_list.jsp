...
<script language="javascript" type="text/javascript">
            ...
            function deleteAdmin(id) {
                ...
                if(r) {
                	window.location.href = "deleteAdmin.do?admin_id="+id;
                }
            }
            //全选
	...
	<div id="main">
		<form action="findAdmin.do" method="post">
			...
			<div class="search_add">
				<div>
					模块： <select name="moduleId" id="selModules" class="select_search">
				...
				<div>
					角色：<input type="text" name="roleName"
						value="${adminPage.roleName }" class="text_search width200" />
				</div>
				<div>
				...
				<input type="button" value="密码重置" class="btn_add"
					...
					class="btn_add" onclick="location.href='toAddAdmin.do';" />
			</div>
			<!--删除和密码重置的操作提示-->
				...
				<table id="datalist">
					...
					<c:forEach items="${admins }" var="admin">
						<tr>
							...
							<td>${admin.email }</td>
							<td><fmt:formatDate value="${admin.enrolldate }"
									pattern="yyyy-MM-dd" /></td>
							<td><a class="summary" onmouseover="showDetail(true,this);"
								onmouseout="showDetail(false,this);"> <c:forEach
										items="${admin.roles }" var="role" varStatus="status">
										<c:choose>
							...
							<td class="td_modi"><input type="button" value="修改"
								class="btn_modify"
								onclick="location.href='toUpdateAdmin.do?id=${admin.admin_id }';" />
								<input type="button" value="删除" class="btn_delete"
								onclick="deleteAdmin(${admin.admin_id });" /></td>
						</tr>
					</c:forEach>
				</table>
			</div>
				...
				<c:forEach begin="1" end="${adminPage.totalPage }" var="p">
					<c:choose>
						<c:when test="${p==adminPage.currentPage }">
							<a href="findAdmin.do?currentPage=${p }" class="current_page">${p
								}</a>
						</c:when>
						<c:otherwise>
