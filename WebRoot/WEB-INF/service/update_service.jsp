		...
		<form action="updateService.do" method="post" class="main_form">
			...
			<div class="input_info">
				<input type="text" name="os_username"
					value="${service.os_username }" readonly class="readonly" />
			</div>
			<div class="text_info clearfix">
				<span>服务器 IP：</span>
			</div>
			<div class="input_info">
				<input type="text" name="unix_host" value="${service.unix_host }"
					readonly class="readonly" />
			</div>
			<div class="text_info clearfix">
				<span>资费类型：</span>
				...
				<select class="width150" name="cost_id">
					<c:forEach items="${costs }" var="cost">
						<option value="${cost.cost_id }"
							<c:if test="${cost.cost_id==service.cost_id }">selected</c:if>>${cost.name
							}</option>
					</c:forEach>
				</select>
			...
			<p>
				业务说明：<br /> 1、修改资费后，点击保存，并未真正修改数据库中的数据；<br /> 2、提交操作到消息队列；<br />
