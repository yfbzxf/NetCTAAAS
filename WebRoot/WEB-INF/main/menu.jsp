...
<ul id="menu">
	...
	<c:forEach items="${allModules }" var="module">
		...
		<c:if test="${module.module_id==4 }">
			<c:choose>
				<c:when test="${currentModule==4 }">
					<li><a href="<%=path%>/account/findAccount.do"
						class="account_on"></a></li>
				</c:when>
				<c:otherwise>
					<li><a href="<%=path%>/account/findAccount.do"
		...
		<c:if test="${module.module_id==5 }">
			<c:choose>
				<c:when test="${currentModule==5 }">
					<li><a href="<%=path%>/service/findService.do"
						class="service_on"></a></li>
				</c:when>
				<c:otherwise>
					<li><a href="<%=path%>/service/findService.do"
		...
		<c:if test="${module.module_id==6 }">
			<li><a href="<%=path%>/bill/findBill.do" class="bill_off"></a></li>
		</c:if>
		<c:if test="${module.module_id==7 }">
			<li><a href="<%=path%>/report/findReport.do" class="report_off"></a></li>
		</c:if>

	</c:forEach>
	<li><a href="<%=path%>/user/userInfo.do" class="information_off"></a></li>
	<li><a href="<%=path%>/user/tomodifyUser.do" class="password_off"></a></li>
</ul>